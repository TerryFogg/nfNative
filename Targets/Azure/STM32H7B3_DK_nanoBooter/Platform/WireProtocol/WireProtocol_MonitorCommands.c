...
int stm32FlashWrite(uint32_t location, uint32_t lengthInBytes, uint8_t *buffer);
int stm32FlashErase(uint32_t location);

int AccessMemory(uint32_t location,
    ...
    case AccessMemory_Check:
        ...
        *(unsigned int *)buffer =
            SUPPORT_ComputeCRC((uint32_t *)location, lengthInBytes, 0);
        // done here
        return true;
...
int Monitor_Reboot(WP_Message *message) {
    ...
    if (cmd != NULL) {
        if (Monitor_Reboot_c_EnterProprietaryBooter ==
            (cmd->m_flags & Monitor_Reboot_c_EnterProprietaryBooter)) {
...
int stm32FlashWrite(uint32_t location,
    uint32_t lengthInBytes,
    uint8_t *buffer) {
    return 1;
}

int stm32FlashErase(uint32_t location) { return 1; }
