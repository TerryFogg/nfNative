...
//
//// Global device address and data buffer
...
//TX_EVENT_FLAGS_GROUP wpConnectedEvent;
//
...
//{
//    // Flag to indicate USB is connected
//    tx_event_flags_create(&wpConnectedEvent, "wpProtocolConnectedEvent");
//    // Flag to indicate USB received some bytes
//    tx_event_flags_create(&wpReceivedBytesEvent, "wpReceivedBytesEvent");
//    wp_InitializeUSB();
//}
//bool wp_WriteBytes(uint8_t *ptr, uint16_t size)
//{
//    usb_start_transfer(ep, ptr, size);
//    return true;
//}
...
//}
//
//// Given an endpoint address, return the usb_endpoint_configuration of that endpoint.
//// Returns NULL if an endpoint of that address is not found.
//// param addr
...
//}
//
//// Set up the endpoint control register for an endpoint,not valid for EP0
//// param ep
...
//}
//
//// Given an endpoint configuration, returns true if the endpoint is transmitting data to the host (i.e. is an IN endpoint)
...
//// hardware is done in ep0_in_handler.
//// This is because we have to acknowledge the request first s a device with address zero.
//// pkt, the setup packet from the host.
...
//}
//
//// Handles a SET_CONFIGRUATION request from the host.
//// Assumes one configuration so simply sends a zero length status packet back to the host.
//// pkt, the setup packet from the host.
...
//}
//
//// Handle a "buffer status" irq. This means that one or more buffers have been sent / received.
