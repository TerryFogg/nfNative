...
#define SPI_MAX_TRANSFER_SIZE                                                  \
  (320 * 2 * NUMBER_OF_LINES) // 320 pixels 2 words wide ( 16 bit colour)
...
void SwapBuffers();
void FlushData();
void CopyData16ByteSwapped(CLR_UINT16 *dest, CLR_UINT16 *src,
                           CLR_UINT32 length);
...
void DisplayInterface::SetCommandMode(int mode) { spiCommandMode = mode; }
...
void DisplayInterface::SendCommand(CLR_UINT8 arg_count, ...) {
  ...
  // Make sure any outstanding async writes have finished before we set the pin
  // low
...
void SwapBuffers() {
  currentBuffer = (currentBuffer == (CLR_UINT16 *)spiBuffer)
                      ? (CLR_UINT16 *)spiBuffer2
                      : (CLR_UINT16 *)spiBuffer;
  bufferPtr = currentBuffer;
  bufferWritten = 0;
}
...
void SendData16(CLR_UINT16 *data, CLR_UINT32 length, bool doByteSwap) {
  while (length > 0) {
    ...
    // If we need to byteswap the data, do that - otherwise we can just memcpy
    // it
    if (doByteSwap) {
...
void DisplayInterface::SendData16Windowed(CLR_UINT16 *data, CLR_UINT32 startX,
                                          CLR_UINT32 startY, CLR_UINT32 width,
                                          CLR_UINT32 height, CLR_UINT32 stride,
                                          bool doByteSwap) {
...
void DisplayInterface::FillData16(CLR_UINT16 fillValue, CLR_UINT32 fillLength) {
  ...
  memset(
      currentBuffer, fillValue,
      ((fillLength < SPI_MAX_TRANSFER_16) ? fillLength : SPI_MAX_TRANSFER_16) *
          2);

  // If our length doesn't evenly divide our buffer size, send out the small
  // buffer first This gives us the maximum freed up time at the end
