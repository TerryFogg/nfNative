...
#define wp_ReadFromBuffer(ptr, size, wait_time)                                \
  wp_ReadFromUsartBuffer(ptr, size, wait_time)
#define wp_WriteToBuffer(payload, size) wp_WriteToUsartBuffer(payload, size)
#else
#define wp_ReadFromBuffer(ptr, size, wait_time)                                \
  wp_ReadFromUsbBuffer(ptr, size, wait_time)
#define wp_WriteToBuffer(payload, size) wp_WriteToUsbBuffer(payload, size)
#endif

void WP_ReceiveBytes(uint8_t **ptr, uint32_t *size) {
  if (*size !=
      0) // Can get 0 for size if all header and payload comes through quickly
  {
