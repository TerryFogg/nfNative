...
CircularBuffer_t
    wp_UsartReceiveCircularBuffer; // Circular buffer instance for Transmit data
...
volatile size_t
    usart_tx_dma_current_len; //  Length of currently active TX DMA transfer
...
void wp_InitializeUsart(void) {
  ...
  GPIO_InitStruct.Pin =
      wpUSART_TX_PIN |
      wpUSART_RX_PIN; // UART TX/RX GPIO pin configuration and clock
  ...
  LL_DMA_SetDataTransferDirection(wpDMA, wpDMA_ReceiveStream,
                                  LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
  LL_DMA_SetStreamPriorityLevel(wpDMA, wpDMA_ReceiveStream,
                                LL_DMA_PRIORITY_LOW);
  LL_DMA_SetMode(wpDMA, wpDMA_ReceiveStream, LL_DMA_MODE_CIRCULAR);
  LL_DMA_SetPeriphIncMode(wpDMA, wpDMA_ReceiveStream,
  ...
  LL_DMA_SetPeriphAddress(
      wpDMA, wpDMA_ReceiveStream,
      LL_USART_DMA_GetRegAddr(wpUSART, LL_USART_DMA_REG_DATA_RECEIVE));
  LL_DMA_SetMemoryAddress(wpDMA, wpDMA_ReceiveStream,
                          (uint32_t)wpUSART_DMA_Receive_Buffer);
  LL_DMA_SetDataLength(wpDMA, wpDMA_ReceiveStream,
                       ARRAY_LEN(wpUSART_DMA_Receive_Buffer));

  // wpUSART Transmit Initialize
  LL_DMA_SetPeriphRequest(wpDMA, wpDMA_TransmitStream, wpDMA_TransmitMux);
  LL_DMA_SetDataTransferDirection(wpDMA, wpDMA_TransmitStream,
                                  LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
  LL_DMA_SetStreamPriorityLevel(wpDMA, wpDMA_TransmitStream,
                                LL_DMA_PRIORITY_LOW);
  LL_DMA_SetMode(wpDMA, wpDMA_TransmitStream, LL_DMA_MODE_NORMAL);
  LL_DMA_SetPeriphIncMode(wpDMA, wpDMA_TransmitStream,
                          LL_DMA_PERIPH_NOINCREMENT);
  ...
  LL_DMA_SetPeriphAddress(
      wpDMA, wpDMA_TransmitStream,
      LL_USART_DMA_GetRegAddr(wpUSART, LL_USART_DMA_REG_DATA_TRANSMIT));

  LL_DMA_EnableIT_HT(
      wpDMA, wpDMA_ReceiveStream); // Enable receive half transfer interrupt
  LL_DMA_EnableIT_TC(
      wpDMA, wpDMA_ReceiveStream); // Enable receive transfer complete interrupt
  LL_DMA_EnableIT_TC(
      wpDMA,
      wpDMA_TransmitStream); // Enable transmit transfer complete interrupt

  // DMA interrupt initialize
  NVIC_SetPriority(wpDMA_ReceiveStreamInterrupt,
                   NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0, 0));
  NVIC_EnableIRQ(wpDMA_ReceiveStreamInterrupt);
  NVIC_SetPriority(wpDMA_TransmitStreamInterrupt,
                   NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0, 0));
  ...
  while (!LL_USART_IsActiveFlag_TEACK(wpUSART) ||
         !LL_USART_IsActiveFlag_REACK(wpUSART)) {
  }
...
bool InitWireProtocolCommunications() {
  wp_InitializeBuffer(&wp_UsartTransmitCircularBuffer, wp_TransmitData,
...
int wp_ReadFromUsartBuffer(uint8_t **ptr, uint32_t *size, uint32_t wait_time) {
  ...
  tx_event_flags_get(&wpUartReceivedBytesEvent, 0x1, TX_OR_CLEAR, &actual_flags,
                     wait_time);
  ULONG read = wp_ReadBuffer(
      &wp_UsartReceiveCircularBuffer, *ptr,
...
void wp_UsartDataReceived(void) {
  ...
  size_t position =
      ARRAY_LEN(wpUSART_DMA_Receive_Buffer) - length_data_received;

  if (position != old_position) {
    if (position > old_position) {
      wp_WriteBuffer(&wp_UsartReceiveCircularBuffer,
                     &wpUSART_DMA_Receive_Buffer[old_position],
                     position -
                         old_position); /* Write data to receive buffer */
    } else {
...
uint8_t wp_UsartStartTxDmaTransfer(void) {
  ...
  if (usart_tx_dma_current_len == 0 &&
      (usart_tx_dma_current_len = length) > 0) {
    ...
    LL_DMA_SetMemoryAddress(wpDMA, wpDMA_TransmitStream,
                            (uint32_t)&wp_UsartTransmitCircularBuffer
                                .buffer[wp_UsartTransmitCircularBuffer.r]);
